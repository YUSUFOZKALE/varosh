                      ...
                      {isExpanded && (
                        <div className="fixed inset-x-0 bottom-0 z-40 bg-neutral-900 border-t-2 border-amber-500/60 rounded-t-2xl overflow-hidden shadow-2xl shadow-black/40 sm:absolute sm:inset-x-0 sm:top-0 sm:bottom-auto sm:rounded-xl sm:border-2">
                          ...
                          <div className="px-3 py-2 space-y-2 max-h-[40vh] overflow-y-auto">
                            ...
                            {(() => {
                              const extras = itemOptionsForItem.filter((o) => o.groupName === "Ekstralar");
                              ...
                              return (
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {extras.map((ext) => (
                                      <button key={ext.id} onClick={() => setItemOptions((prev) => prev.includes(ext.id) ? prev.filter((id) => id !== ext.id) : [...prev, ext.id])} className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${itemOptions.includes(ext.id) ? "bg-amber-500/20 text-amber-300 border border-amber-500/40" : "bg-neutral-800 text-white/70 border border-neutral-700/50"}`}>
                                        {ext.optionName} {ext.priceModifier > 0 && <span className="text-white/30">+{ext.priceModifier.toFixed(0)}</span>}
                                      </button>
                                    ))}
                                  </div>
                                </div>
