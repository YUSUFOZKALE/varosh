    ...
    <div className="max-w-2xl mx-auto p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <input
          ...
          className="bg-surface-1 border border-border rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-accent/50"
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4">
          <p className="text-green-400/60 text-xs font-medium">Nakit Gelir</p>
          <p className="text-2xl font-bold text-green-400">{(summary?.cashIncome || 0).toFixed(0)} <span className="text-sm font-normal">TL</span></p>
        </div>
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4">
          <p className="text-blue-400/60 text-xs font-medium">Kart / POS Gelir</p>
          <p className="text-2xl font-bold text-blue-400">{(summary?.cardIncome || 0).toFixed(0)} <span className="text-sm font-normal">TL</span></p>
        </div>
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4">
          <p className="text-red-400/60 text-xs font-medium">Toplam Gider</p>
          <p className="text-2xl font-bold text-red-400">{(summary?.totalExpense || 0).toFixed(0)} <span className="text-sm font-normal">TL</span></p>
        </div>
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4">
          <p className="text-amber-400/60 text-xs font-medium">Net Bakiye</p>
          <p className="text-2xl font-bold text-amber-400">{(summary?.netBalance || 0).toFixed(0)} <span className="text-sm font-normal">TL</span></p>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 gap-3">
          <div className="bg-surface-1 border border-border rounded-2xl p-4">
            ...
            <p className="text-3xl font-bold text-white">{summary.netCashBalance.toFixed(0)} <span className="text-sm font-normal text-white/40">TL</span></p>
          ...
          {summary.courierCashOnHand > 0 && (
            <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-2xl p-4">
              <div className="flex items-center justify-between">
                ...
                <p className="text-2xl font-bold text-cyan-400">{summary.courierCashOnHand.toFixed(0)} <span className="text-sm font-normal">TL</span></p>
              </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={() => openModal("income")} className="py-3.5 rounded-2xl bg-green-600/20 border border-green-500/30 text-green-300 font-bold text-sm transition-all active:scale-[0.97] flex items-center justify-center gap-2">
        ...
        <button onClick={() => openModal("expense")} className="py-3.5 rounded-2xl bg-red-600/20 border border-red-500/30 text-red-300 font-bold text-sm transition-all active:scale-[0.97] flex items-center justify-center gap-2">
        ...
        <button onClick={() => openModal("trust")} className="py-3.5 rounded-2xl bg-purple-600/20 border border-purple-500/30 text-purple-300 font-bold text-sm transition-all active:scale-[0.97] flex items-center justify-center gap-2">
        ...
        <button onClick={() => openModal("staff_payment")} className="py-3.5 rounded-2xl bg-orange-600/20 border border-orange-500/30 text-orange-300 font-bold text-sm transition-all active:scale-[0.97] flex items-center justify-center gap-2">
      ...
      <div className="flex gap-1 bg-surface-1 rounded-xl p-1">
        {(["overview", "transactions", "trust", "dayclose"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${tab === t ? "bg-accent text-black" : "text-white/40"}`}
          >
      ...
      {tab === "trust" && (
        <div className="space-y-4">
          <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4 text-center">
            ...
            <p className="text-3xl font-bold text-purple-400">{totals.trustOutstanding.toFixed(0)} TL</p>
          </div>
          ...
          {trustActive.length > 0 && (
            <div className="space-y-2">
              ...
              {trustActive.map((tx) => (
                <div key={tx.id} className="bg-surface-1 rounded-xl p-3 border border-purple-500/20 flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="text-white font-medium text-sm">{tx.person}</p>
                    <p className="text-white/40 text-[11px]">{tx.description || "-"} · {formatTime(tx.created_at)}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-purple-400 font-bold">{tx.amount.toFixed(0)} TL</span>
                    <button
          ...
          <div className="bg-surface-1 rounded-2xl border border-border p-4 space-y-3">
            ...
            <div className="pt-2 border-t border-white/10">
              <div className="flex justify-between items-center">
                <span className="text-white font-bold">Net Bakiye</span>
                <span className="text-2xl font-bold text-amber-400">{summary.netBalance.toFixed(0)} TL</span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-white/40 text-sm">Kasadaki Nakit</span>
                <span className="text-lg font-bold text-white">{summary.netCashBalance.toFixed(0)} TL</span>
              </div>
            </div>
          </div>
